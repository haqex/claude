...
### 2. Parse arguments
- First argument is the pipeline name (e.g. `design-system`)
- Remaining arguments are `key=value` input pairs (e.g. `product="fintech SaaS"`)
...
- If any required input is missing, output what's needed and stop:
  ...
  Missing required inputs:
    - product: What type of product to design for

  Usage: run-pipeline design-system product=<description>
  ```
